...
### Manual score breakdown (log form)

The Sprawlopolis log form optionally captures the same breakdown that imports get. Field pieces: a `cardpoints` field (per-card points, inputs labeled by the picked cards, keyed by card option string so they survive reselection; writes `cpts1/2/3` + `sumTo: sheet_scoring`), zone number fields (`sheet_residential/…`) tagged `scorePart:'add'`, and `sheet_roadtax` tagged `scorePart:'sub'`. The `score` field is marked `derived:true`: `recomputeDerivedScore()` auto-fills it from the breakdown parts whenever a breakdown field changes, but it stays editable, so a quick score-only log still works (auto-fill only fires when a breakdown field is touched). Empty breakdown fields are left unset on save (not written as 0) so they don't dilute the `avg presentOnly` breakdown stats. Manually-entered per-card points (`cpts1/2/3`, aligned to `card1/2/3`) enable per-card value analysis that imports can't provide (scoresheet rows are slots, not card identities).

### Scoresheet breakdown import

`parseScoresheetBreakdown(play)` extracts the point breakdown from a BGStats scoresheet (Sprawlopolis and Agropolis share the format) into `sheet_<templateId>` fields: e.g. `sheet_residential/commercial/industrial/park` + `sheet_roadtax` (Sprawlopolis), `sheet_cornfeld/orchard/vineyard/livestock` + `sheet_roadtax` (Agropolis — note BGStats' `cornfeld` typo), plus `sheet_scoring` = sum of the "Scoring cards" rows. Verified against real exports: components reconcile to the final score (33+15−8=40; 12+23−5=30). **Card identity is NOT in the scoresheet** — scoring rows are slots 1/2/3, so points can't be attributed to specific cards; only aggregate breakdown is imported. Surfaced via the `avg` stat type (new `presentOnly: true` flag skips plays lacking the field, e.g. in-app manual logs, so they don't dilute averages). Sprawlopolis shows these under a `customStats2` "Score Breakdown" section.
